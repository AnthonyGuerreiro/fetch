...
public class TestTransmissionManager {
	...
	private void removeTorrent(TorrentInfo info, boolean cleanup) throws DownloadManagerException {
		if (!cleanup) {
			...
			try {
				...
				String msg = "Failed to remove torrent with id(" + info.getId() + ") and name (" + info.getName()
						+ ") in test cleanup phase";
				throw new DownloadManagerException(msg, e);
			}
		}
	}
